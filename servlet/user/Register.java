...
import services.ServiceActivation;
		...
		try{
			...
			if (!request.getParameter("password").equals(""))
				if (request.getParameter("password").equals(request.getParameter("repeatPassword"))) {
					...
					ServiceActivation sActivation=new ServiceActivation();
					Activation activation=new Activation(UUID.randomUUID().toString(),user);
					sActivation.create(activation);
					
					Mail mail=new Mail(getServletContext().getInitParameter("mail"), getServletContext().getInitParameter("passwordMail"), user.getEmail(),"Confirm account", mail(activation.getToken()));
					mail.sendMail();
				}else
